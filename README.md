...
> Status: v0.1.1 public preview. Validate with scan and dry-run before applying any cleanup.
...
The public preview also provides a universal `arm64` + `x86_64` archive and SHA-256 checksum on the [v0.1.1 release page](https://github.com/minhnh510/keepitclean/releases/tag/v0.1.1):

```bash
curl -LO https://github.com/minhnh510/keepitclean/releases/download/v0.1.1/keepitclean-v0.1.1-macos-universal.tar.gz
curl -LO https://github.com/minhnh510/keepitclean/releases/download/v0.1.1/keepitclean-v0.1.1-macos-universal.tar.gz.sha256
shasum -a 256 -c keepitclean-v0.1.1-macos-universal.tar.gz.sha256
tar -xzf keepitclean-v0.1.1-macos-universal.tar.gz
cd keepitclean-v0.1.1-macos-universal
./scripts/install-release.sh
```

Release binaries are ad-hoc signed and not Apple-notarized in v0.1.1. Verify the published checksum, or build from source if your Gatekeeper policy requires a notarized Developer ID artifact. The archive installer remains user-scoped and refuses to overwrite an unrelated `keep` command.
...
Protected examples include the Codex session/archive roots, their latest seven day buckets and every individual session/archive file; Codex memories, SQLite databases, credentials and worktree state; Android AVD userdata/config; Colima disks and volumes; local Maven artifacts; private CocoaPods repositories; and arbitrary Downloads content. Only a complete old Codex `YYYY/MM/DD` session bucket or dated `YYYY-through-MM-DD` archive bundle can cross the protected-root boundary, and only after the current hardcore rule and inactive-process state are revalidated.
